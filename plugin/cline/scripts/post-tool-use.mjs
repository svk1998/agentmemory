...
//#endregion
//#region src/hooks/cline/post-tool-use.ts
async function run(input, ctx) {
	const success = input.success !== false;
	ctx.postDetached("/observe", observePayload(input, success ? "post_tool_use" : "post_tool_failure", {
		tool_name: input.tool,
		tool_input: truncate(input.parameters, 4e3),
		tool_output: truncate(input.result, 8e3),
